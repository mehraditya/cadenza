"""Depth-Anything-V2-Small modality for the G1 forward camera.

Same backbone as the Go1 variant; named separately so per-robot tunings
(crop region, region-mean bands) can diverge later.
"""
...
class Depth(Modality):
    """Monocular depth from depth-anything/Depth-Anything-V2-Small-hf for G1."""

    name = "g1_depth"
    description = "Depth-Anything-V2-Small monocular depth for G1."
    ...
    def compute(self, observation) -> ModalityResult:
        ...
        # G1 stands taller than Go1; bias the crop band higher in the frame.
        h, w = depth_map.shape[:2]
        ymin, ymax = int(h * 0.35), int(h * 0.80)
        third = w // 3
